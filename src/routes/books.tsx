...
export const Route = createFileRoute("/books")({
  head: () => ({
    meta: [
      { title: "Books — Pavulum" },
      { name: "description", content: "A small, returning library — books on attention, love, parenting, and the examined life." },
      { property: "og:title", content: "Books — Pavulum" },
      { property: "og:description", content: "Books we return to, gathered by Pavulum." },
    ],
  }),
  component: BooksPage,
});
    ...
    <main className="bg-background text-foreground">
      ...
      <section className="mx-auto max-w-[1400px] px-6 pb-20 pt-40 md:px-10 md:pt-48">
        ...
        <p className="mt-6 max-w-xl text-base leading-relaxed text-muted-foreground">
          Each title is chosen for the way it changes the temperature of a room — slower thinking, warmer attention.
        </p>
        <div className="mt-20 grid gap-12 md:grid-cols-3 md:gap-10">
          {library.map((b) => (
            <article key={b.title} className="group">
              <div className="relative aspect-[3/4] overflow-hidden bg-beige">
                <img src={b.img} alt={b.title} loading="lazy" className="img-cinematic h-full w-full object-cover transition-transform duration-[1400ms] ease-out group-hover:scale-[1.04]" />
                {b.tag !== "—" && (
                  <span className="absolute left-4 top-4 bg-ivory/90 px-3 py-1 text-[10px] uppercase tracking-[0.22em] text-charcoal">{b.tag}</span>
                )}
              </div>
